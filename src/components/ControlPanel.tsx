...
export function Controls({ uniforms }: {
    ...
    return <>
        {Object.keys(uniforms).map((name: string) => {
            ...
            switch (uniform.type) {
                case "radio":
                    return <RadioControl name={name} uniform={uniform} key={name} />;
                case "slider":
                    return <SliderControl name={name} uniform={uniform} key={name} />;
                case "group":
                    return <GroupControl name={name} uniform={uniform} key={name} />;
                case "checkbox":
...
function SettingsControls<UC extends UniformControls>({ sketcher, settings }: {
